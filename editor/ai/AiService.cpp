...
std::string AiService::buildSystemPrompt() const {
    ...
    prompt
        ...
        << "A bundle is only built and registered when a scene instantiates it or when it is listed as standalone, so a bundle that exists only to be spawned from a script with BundleManager needs set_standalone_bundle before createBundle can find it by name.\n"
